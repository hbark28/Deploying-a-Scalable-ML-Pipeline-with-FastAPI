...
class BLISController(LibController):
    ...
    def get_num_threads(self):
        get_func = getattr(
            self.dynlib, "bli_thread_get_num_threads", lambda: None)
...
class FlexiBLASController(LibController):
    ...
    def get_num_threads(self):
        get_func = getattr(
            self.dynlib, "flexiblas_get_num_threads", lambda: None)
        num_threads = get_func()
        # by default BLIS is single-threaded and get_num_threads
        # returns -1. We map it to 1 for consistency with other libraries.
        return 1 if num_threads == -1 else num_threads
    ...
    def get_version(self):
        ...
        minor = ctypes.c_int()
        patch = ctypes.c_int()
        get_version_(ctypes.byref(major), ctypes.byref(
            minor), ctypes.byref(patch))
    ...
    def switch_backend(self, backend):
        ...
        if backend not in self.loaded_backends:
            if backend in self.available_backends:
                load_func = getattr(
                    self.dynlib, "flexiblas_load_backend", lambda _: -1)
...
class ThreadpoolController:
        ...
        try:
            ...
            for h_module in h_modules:
                ...
                if not ps_api.GetModuleFileNameExW(
                    h_process, h_module, ctypes.byref(
                        buf), ctypes.byref(n_size)
                ):
    ...
    def _make_controller_from_path(self, filepath):
        ...
        for controller_class in _ALL_CONTROLLERS:
            ...
            prefix = self._check_prefix(
                filename, controller_class.filename_prefixes)
    ...
    def _warn_if_incompatible_openmp(self):
        ...
        prefixes = [
            lib_controller.prefix for lib_controller in self.lib_controllers]
        msg = textwrap.dedent(
            """

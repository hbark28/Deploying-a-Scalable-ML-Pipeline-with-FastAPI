...
async def read_request(stream: asyncio.StreamReader) -> Tuple[str, Headers]:
    ...
    try:
        ...
        raise EOFError(
            "connection closed while reading HTTP request line") from exc

    try:
        method, raw_path, version = request_line.split(b" ", 2)
    except ValueError:  # not enough values to unpack (expected 3, got 1-2)
        raise ValueError(
            f"invalid HTTP request line: {d(request_line)}") from None
...
async def read_response(stream: asyncio.StreamReader) -> Tuple[int, str, Headers]:
    ...
    try:
        ...
        raise EOFError(
            "connection closed while reading HTTP status line") from exc

    try:
        version, raw_status_code, raw_reason = status_line.split(b" ", 2)
    except ValueError:  # not enough values to unpack (expected 3, got 1-2)
        raise ValueError(
            f"invalid HTTP status line: {d(status_line)}") from None
    ...
    try:
        ...
        raise ValueError(
            f"invalid HTTP status code: {d(raw_status_code)}") from None
...
async def read_headers(stream: asyncio.StreamReader) -> Headers:
    ...
    for _ in range(MAX_HEADERS + 1):
        try:
            ...
            raise EOFError(
                "connection closed while reading HTTP headers") from exc
        if line == b"":
            break

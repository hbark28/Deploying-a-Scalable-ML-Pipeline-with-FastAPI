...
class TestScalars:
    ...
    def test_bytes(self):
        ...
        assert_identical(s.s, np.bytes_(
    ...
    def test_complex64(self):
        s = readsav(path.join(DATA_PATH, 'scalar_complex64.sav'),
                    verbose=False)
        assert_identical(s.c64, np.complex128(
            1.1987253647623157e+112-5.1987258887729157e+307j))
...
class TestCompressed(TestScalars):
    ...
    def test_compressed(self):
        ...
        assert_identical(s.arrays.b[0], np.array(
            [4., 5., 6., 7.], dtype=np.float32))
        assert_identical(s.arrays.c[0], np.array(
            [np.complex64(1+2j), np.complex64(7+8j)]))
        assert_identical(s.arrays.d[0], np.array(
            [b"cheese", b"bacon", b"spam"], dtype=object))
...
class TestStructures:
    ...
    def test_scalars_replicated_3d(self):
        ...
        assert_identical(s.scalars_rep.a, np.repeat(
            np.int16(1), 24).reshape(4, 3, 2))
        assert_identical(s.scalars_rep.b, np.repeat(
            np.int32(2), 24).reshape(4, 3, 2))
        assert_identical(s.scalars_rep.c, np.repeat(
            np.float32(3.), 24).reshape(4, 3, 2))
        assert_identical(s.scalars_rep.d, np.repeat(
            np.float64(4.), 24).reshape(4, 3, 2))
        assert_identical(s.scalars_rep.e, np.repeat(
            b"spam", 24).reshape(4, 3, 2).astype(object))
        assert_identical(s.scalars_rep.f, np.repeat(
            np.complex64(-1.+3j), 24).reshape(4, 3, 2))

    def test_arrays(self):
        s = readsav(path.join(DATA_PATH, 'struct_arrays.sav'), verbose=False)
        assert_array_identical(
            s.arrays.a[0], np.array([1, 2, 3], dtype=np.int16))
        assert_array_identical(s.arrays.b[0], np.array(
            [4., 5., 6., 7.], dtype=np.float32))
        assert_array_identical(s.arrays.c[0], np.array(
            [np.complex64(1+2j), np.complex64(7+8j)]))
        assert_array_identical(s.arrays.d[0], np.array(
            [b"cheese", b"bacon", b"spam"], dtype=object))

    def test_arrays_replicated(self):
        s = readsav(
            path.join(DATA_PATH, 'struct_arrays_replicated.sav'), verbose=False)
...
class TestPointers:
    ...
    def test_pointers(self):
        s = readsav(
            path.join(DATA_PATH, 'scalar_heap_pointer.sav'), verbose=False)
        assert_identical(s.c64_pointer1, np.complex128(
            1.1987253647623157e+112-5.1987258887729157e+307j))
        assert_identical(s.c64_pointer2, np.complex128(
            1.1987253647623157e+112-5.1987258887729157e+307j))
...
class TestPointerArray:
    ...
    def test_1d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_1d.sav'), verbose=False)
        assert_equal(s.array1d.shape, (123, ))
    ...
    def test_2d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_2d.sav'), verbose=False)
        assert_equal(s.array2d.shape, (22, 12))
    ...
    def test_3d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_3d.sav'), verbose=False)
        assert_equal(s.array3d.shape, (11, 22, 12))
    ...
    def test_4d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_4d.sav'), verbose=False)
        assert_equal(s.array4d.shape, (4, 5, 8, 7))
    ...
    def test_5d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_5d.sav'), verbose=False)
        assert_equal(s.array5d.shape, (4, 3, 4, 6, 5))
    ...
    def test_6d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_6d.sav'), verbose=False)
        assert_equal(s.array6d.shape, (3, 6, 4, 5, 3, 4))
    ...
    def test_7d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_7d.sav'), verbose=False)
        assert_equal(s.array7d.shape, (2, 1, 2, 3, 4, 3, 2))
        assert_(np.all(s.array7d == np.float32(4.)))
        assert_(np.all(vect_id(s.array7d) == id(
            s.array7d[0, 0, 0, 0, 0, 0, 0])))

    def test_8d(self):
        s = readsav(
            path.join(DATA_PATH, 'array_float32_pointer_8d.sav'), verbose=False)
        assert_equal(s.array8d.shape, (4, 3, 2, 1, 2, 3, 5, 4))
        assert_(np.all(s.array8d == np.float32(4.)))
        assert_(np.all(vect_id(s.array8d) == id(
            s.array8d[0, 0, 0, 0, 0, 0, 0, 0])))
...
class TestPointerStructures:
    ...
    def test_scalars(self):
        ...
        assert_identical(s.pointers.g, np.array(
            np.float32(4.), dtype=np.object_))
        assert_identical(s.pointers.h, np.array(
            np.float32(4.), dtype=np.object_))
    ...
    def test_pointers_replicated(self):
        ...
        assert_identical(s.pointers_rep.g, np.repeat(
            np.float32(4.), 5).astype(np.object_))
        assert_identical(s.pointers_rep.h, np.repeat(
            np.float32(4.), 5).astype(np.object_))
    ...
    def test_pointers_replicated_3d(self):
        ...
        s_expect = np.repeat(np.float32(4.), 24).reshape(
            4, 3, 2).astype(np.object_)
    ...
    def test_arrays(self):
        s = readsav(
            path.join(DATA_PATH, 'struct_pointer_arrays.sav'), verbose=False)
        assert_array_identical(s.arrays.g[0], np.repeat(
            np.float32(4.), 2).astype(np.object_))
        assert_array_identical(s.arrays.h[0], np.repeat(
            np.float32(4.), 3).astype(np.object_))
    ...
    def test_arrays_replicated(self):
        ...
        # Check values
        for i in range(5):
            assert_array_identical(s.arrays_rep.g[i], np.repeat(
                np.float32(4.), 2).astype(np.object_))
            assert_array_identical(s.arrays_rep.h[i], np.repeat(
                np.float32(4.), 3).astype(np.object_))
            assert_(
        ...
        for i in range(4):
            for j in range(3):
                for k in range(2):
                    ...
                    assert_(np.all(vect_id(s.arrays_rep.g[i, j, k]) == id(
                        s.arrays_rep.g[0, 0, 0][0])))
                    assert_(np.all(vect_id(s.arrays_rep.h[i, j, k]) == id(
...
class TestTags:
    ...
    def test_description(self):
        s = readsav(path.join(DATA_PATH, 'scalar_byte_descr.sav'),
                    verbose=False)
        assert_identical(s.i8u, np.uint8(234))

...
def _gen_gaussians_even(sigmas, total_length):
    ...
    center_locs = np.linspace(delta, total_length -
                              delta, num=num_peaks).astype(int)
...
class TestRidgeLines:
    ...
    def test_single_pass(self):
        ...
        length = 12
        line = _gen_ridge_line([0, 25], test_matr.shape,
                               length, distances, gaps)
...
class TestPeakProminences:
    ...
    def test_wlen(self):
        ...
        for wlen, i in [(8, 0), (7, 0), (6, 0), (5, 1), (3.2, 1), (3, 2), (1.1, 2)]:
            assert_equal(peak_prominences(x, peak, wlen),
                         [3. - i, 0 + i, 6 - i])
...
def test_unpack_condition_args():
    ...
    amin_calc, amax_calc = _unpack_condition_args(
        (amin_true, amax_true), x, peaks)

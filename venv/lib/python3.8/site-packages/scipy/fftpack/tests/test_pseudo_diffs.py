...
class TestDiff:
    ...
    def test_definition(self):
        for n in [16, 17, 64, 127, 32]:
            ...
            assert_array_almost_equal(
                diff(sin(2*x), 3), direct_diff(sin(2*x), 3))
            assert_array_almost_equal(
                diff(sin(2*x), 4), direct_diff(sin(2*x), 4))
            ...
            assert_array_almost_equal(
                diff(sin(x*n/8)), direct_diff(sin(x*n/8)))
            assert_array_almost_equal(
                diff(cos(x*n/8)), direct_diff(cos(x*n/8)))
            for k in range(5):
                assert_array_almost_equal(
    ...
    def test_random_even(self):
        for h in [0.1, 0.5, 1, 5.5, 10]:
            for n in [32, 64, 56]:
                ...
                assert_almost_equal(sum(f, axis=0), 0.0)
                assert_array_almost_equal(
                    direct_tilbert(direct_itilbert(f, h), h), f)
...
class TestShift:
    ...
    def test_definition(self):
        for n in [18, 17, 64, 127, 32, 2048, 256]:
            ...
            for a in [0.1, 3]:
                assert_array_almost_equal(
                    shift(sin(x), a), direct_shift(sin(x), a))

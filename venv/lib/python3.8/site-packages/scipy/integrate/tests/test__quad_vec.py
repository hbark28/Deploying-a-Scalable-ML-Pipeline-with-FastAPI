...
def test_quad_vec_simple_inf(quadrature):
    def f(x): return 1 / (1 + np.float64(x)**2)
    ...
    def f(x): return np.sin(x + 2) / (1 + x**2)
...
def test_quad_vec_args():
    def f(x, a): return x * (x + a) * np.arange(3)
    a = 2
...
def test_quad_vec_pool():
    ...
    with Pool(10) as pool:
        ...
        res, err = quad_vec(f, -np.inf, np.inf, norm='max',
                            epsabs=1e-4, workers=pool.map)
        assert_allclose(res, np.pi, rtol=0, atol=1e-4)
...
def test_num_eval(quadrature):
    ...
    count = [0]
    res = quad_vec(f, 0, 1, norm='max', full_output=True,

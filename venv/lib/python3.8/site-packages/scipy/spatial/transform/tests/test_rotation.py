...
def test_rotvec_calc_pipeline():
    ...
    assert_allclose(Rotation.from_rotvec(
        rotvec, degrees=True).as_rotvec(degrees=True), rotvec)
...
def test_past_180_degree_rotation():
    ...
    assert_array_almost_equal(Rotation.from_euler(
        'xyz', [270, 0, 0], degrees=True).as_mrp(), expected_mrp)
...
def test_getitem():
    ...
    assert_allclose(r[:-1].as_matrix(),
                    np.expand_dims(mat[0], axis=0), atol=1e-15)

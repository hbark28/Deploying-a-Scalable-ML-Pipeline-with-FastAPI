...
class TestInt32Overflow:
    ...
    def setup_method(self):
        ...
        # same time
        try:
            parallel_count = int(os.environ.get(
    ...
    def test_bsr_1_block(self, op):
        ...
        def get_matrix():
            ...
            m = bsr_matrix((data, indices, indptr),
                           blocksize=(n, n), copy=False)
    ...
    def _check_bsr_matmat(self, m):
        ...
        # _bsr_matmat
        m2 = bsr_matrix(np.ones((n, 2), dtype=np.int8),
                        blocksize=(m.blocksize[1], 2))
        m.dot(m2)  # shouldn't SIGSEGV
        del m2

        # _bsr_matmat
        m2 = bsr_matrix(np.ones((2, n), dtype=np.int8),
                        blocksize=(2, m.blocksize[0]))

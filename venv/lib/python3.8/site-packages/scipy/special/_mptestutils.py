...
def get_args(argspec, n):
    if isinstance(argspec, np.ndarray):
        ...
        nargs = len(argspec)
        ms = np.asarray([1.5 if isinstance(spec, ComplexArg)
                        else 1.0 for spec in argspec])
        ms = (n**(ms/sum(ms))).astype(int) + 1

        args = [spec.values(m) for spec, m in zip(argspec, ms)]
        args = np.array(np.broadcast_arrays(
            *np.ix_(*args))).reshape(nargs, -1).T
...
class MpmathData:
    def __init__(self, scipy_func, mpmath_func, arg_spec, name=None,
        ...
        if isinstance(self.arg_spec, np.ndarray):
            self.is_complex = np.issubdtype(
                self.arg_spec.dtype, np.complexfloating)
        else:
            self.is_complex = any([isinstance(arg, ComplexArg)
            ...
            for j, dps in enumerate(dps_list):
                ...
                try:
                    assert_func_equal(self.scipy_func,
                                      lambda *a: pytype(self.mpmath_func(*
                                                        map(mptype, a))),
                                      argarr,
                                      vectorized=False,

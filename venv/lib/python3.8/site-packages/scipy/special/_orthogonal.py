...
class orthopoly1d(np.poly1d):
    ...
    def __init__(self, roots, weights=None, hn=1.0, kn=1.0, wfunc=None,
        ...
        if monic:
            ...
            if evf:
                knn = kn
                def eval_func(x): return evf(x) / knn
            mu = mu / abs(kn)
            kn = 1.0
...
def roots_jacobi(n, alpha, beta, mu=False):
    ...
    b = beta
    if a + b == 0.0:
        def an_func(k): return np.where(k == 0, (b-a)/(2+a+b), 0.0)
    else:
        def an_func(k): return np.where(k == 0, (b-a)/(2+a+b),
                                        (b*b - a*a) / ((2.0*k+a+b)*(2.0*k+a+b+2)))

    def bn_func(k): return 2.0 / (2.0*k+a+b)*np.sqrt((k+a)*(k+b) / (2*k+a+b+1)) \
        * np.where(k == 1, 1.0, np.sqrt(k*(k+a+b) / (2.0*k+a+b-1)))

    def f(n, x): return _ufuncs.eval_jacobi(n, a, b, x)
    def df(n, x): return (0.5 * (n + a + b + 1)
...
def jacobi(n, alpha, beta, monic=False):
    ...
    def wfunc(x): return (1 - x)**alpha * (1 + x)**beta
    if n == 0:
...
def sh_jacobi(n, p, q, monic=False):
    ...
    def wfunc(x): return (1.0 - x)**(p - q) * (x)**(q - 1.)
    if n == 0:
        return orthopoly1d([], [], 1.0, 1.0, wfunc, (-1, 1), monic,
                           eval_func=np.ones_like)
    n1 = n
...
def roots_genlaguerre(n, alpha, mu=False):
    ...
    def an_func(k): return 2 * k + alpha + 1
    def bn_func(k): return -np.sqrt(k * (k + alpha))
    def f(n, x): return _ufuncs.eval_genlaguerre(n, alpha, x)
    def df(n, x): return (n*_ufuncs.eval_genlaguerre(n, alpha, x)
...
def genlaguerre(n, alpha, monic=False):
    ...
    def wfunc(x): return exp(-x) * x**alpha
    if n == 0:
        x, w = [], []
...
def roots_hermite(n, mu=False):
    ...
    mu0 = np.sqrt(np.pi)
    if n <= 150:
        def an_func(k): return 0.0*k
        def bn_func(k): return np.sqrt(k/2.0)
        f = _ufuncs.eval_hermite
        def df(n, x): return 2.0 * n * _ufuncs.eval_hermite(n-1, x)
...
def _compute_tauk(n, k, maxit=5):
    ...
    def f(x): return x - sin(x) - c
    def df(x): return 1.0 - cos(x)
    xi = 0.5*pi
...
def _pbcf(n, theta):
    ...
    u3 = (-4042.0*ctp[9, :] + 18189.0*ctp[7, :] - 28287.0 *
          ctp[5, :] - 151995.0*ctp[3, :] - 259290.0*ct) / 414720.0
    ...
    v3 = (-4042.0*ctp[9, :] + 18189.0*ctp[7, :] - 36387.0 *
          ctp[5, :] + 238425.0*ctp[3, :] + 259290.0*ct) / 414720.0
    ...
    A2 = (b4*u0 + phip[0, :]*b3*u1 + phip[1, :]*b2*u2 +
          phip[2, :]*b1*u3 + phip[3, :]*b0*u4) / zeta**6
    B0 = -(a1*u0 + phip[0, :]*a0*u1) / zeta**2
    B1 = -(a3*u0 + phip[0, :]*a2*u1 + phip[1, :]
           * a1*u2 + phip[2, :]*a0*u3) / zeta**5
    B2 = -(a5*u0 + phip[0, :]*a4*u1 + phip[1, :]*a3*u2 + phip[2, :]
           * a2*u3 + phip[3, :]*a1*u4 + phip[4, :]*a0*u5) / zeta**8
    # U
    ...
    C1 = -(b3*v0 + phip[0, :]*b2*v1 + phip[1, :]
           * b1*v2 + phip[2, :]*b0*v3) / zeta**4
    C2 = -(b5*v0 + phip[0, :]*b4*v1 + phip[1, :]*b3*v2 + phip[2, :]
           * b2*v3 + phip[3, :]*b1*v4 + phip[4, :]*b0*v5) / zeta**7
    D0 = a0*v0
    D1 = (a2*v0 + phip[0, :]*a1*v1 + phip[1, :]*a0*v2) / zeta**3
    D2 = (a4*v0 + phip[0, :]*a3*v1 + phip[1, :]*a2*v2 +
          phip[2, :]*a1*v3 + phip[3, :]*a0*v4) / zeta**6
    # Derivative of U
...
def roots_hermitenorm(n, mu=False):
    ...
    if n <= 150:
        def an_func(k): return 0.0*k
        def bn_func(k): return np.sqrt(k)
        f = _ufuncs.eval_hermitenorm
        def df(n, x): return n * _ufuncs.eval_hermitenorm(n-1, x)
        return _gen_roots_and_weights(m, mu0, an_func, bn_func, f, df, True, mu)
    else:
        nodes, weights = _roots_hermite_asy(m)
        # Transform
...
def hermitenorm(n, monic=False):
    ...
    def wfunc(x): return exp(-x * x / 2.0)
    if n == 0:
        x, w = [], []
...
def roots_gegenbauer(n, alpha, mu=False):
    ...
    def an_func(k): return 0.0 * k
    def bn_func(k): return np.sqrt(k * (k + 2 * alpha - 1)
                                   / (4 * (k + alpha) * (k + alpha - 1)))

    def f(n, x): return _ufuncs.eval_gegenbauer(n, alpha, x)

    def df(n, x): return ((-n*x*_ufuncs.eval_gegenbauer(n, alpha, x)
                           + ((n + 2*alpha - 1)
...
def chebyt(n, monic=False):
    ...
    def wfunc(x): return 1.0 / sqrt(1 - x * x)
    if n == 0:
...
def roots_legendre(n, mu=False):
    ...
    mu0 = 2.0
    def an_func(k): return 0.0 * k
    def bn_func(k): return k * np.sqrt(1.0 / (4 * k * k - 1))
    f = _ufuncs.eval_legendre
    def df(n, x): return (-n*x*_ufuncs.eval_legendre(n, x)

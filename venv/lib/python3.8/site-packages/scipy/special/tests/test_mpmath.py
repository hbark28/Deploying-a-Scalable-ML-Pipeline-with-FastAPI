...
def test_erf_complex():
    ...
    try:
        ...
        x2, y2 = np.meshgrid(np.logspace(-80, .8, 31),
                             np.logspace(-80, .8, 11))
        points = np.r_[x1.ravel(), x2.ravel()] + 1j * \
            np.r_[y1.ravel(), y2.ravel()]
...
def test_digamma_roots():
    ...
    dx = np.r_[-0.24, -np.logspace(-1, -15, 10),
               0, np.logspace(-15, -1, 10), 0.24]
    dy = dx.copy()
    dx, dy = np.meshgrid(dx, dy)
    dz = dx + 1j*dy
...
class TestSystematic:
    ...
    def test_bei(self):
        assert_mpmath_equal(sc.bei,
                            exception_to_nan(
                                lambda z: mpmath.bei(0, z, **HYPERKW)),
                            [Arg(-1e3, 1e3)])

    def test_ber(self):
        assert_mpmath_equal(sc.ber,
                            exception_to_nan(
                                lambda z: mpmath.ber(0, z, **HYPERKW)),
                            [Arg(-1e3, 1e3)])
    ...
    def test_besseli(self):
        assert_mpmath_equal(sc.iv,
                            exception_to_nan(
                                lambda v, z: mpmath.besseli(v, z, **HYPERKW)),
                            [Arg(-1e100, 1e100), Arg()],
                            atol=1e-270)
    ...
    def test_besselk_complex(self):
        assert_mpmath_equal(lambda v, z: sc.kv(v.real, z),
                            exception_to_nan(
                                lambda v, z: mpmath.besselk(v, z, **HYPERKW)),
                            [Arg(-1e100, 1e100), ComplexArg()])
    ...
    def test_chebyt_int(self):
        assert_mpmath_equal(lambda n, x: sc.eval_chebyt(int(n), x),
                            exception_to_nan(
                                lambda n, x: mpmath.chebyt(n, x, **HYPERKW)),
                            [IntArg(), Arg()], dps=50)
    ...
    def test_chebyu_int(self):
        assert_mpmath_equal(lambda n, x: sc.eval_chebyu(int(n), x),
                            exception_to_nan(
                                lambda n, x: mpmath.chebyu(n, x, **HYPERKW)),
                            [IntArg(), Arg()], dps=50)
    ...
    def test_chi(self):
        ...
        assert_mpmath_equal(
            chi, mpmath.chi, [FixedArg([88 - 1e-9, 88, 88 + 1e-9])])
    ...
    def test_exprel(self):
        assert_mpmath_equal(sc.exprel,
                            lambda x: mpmath.expm1(
                                x)/x if x != 0 else mpmath.mpf('1.0'),
    ...
    def test_log_ndtr(self):
        assert_mpmath_equal(sc.log_ndtr,
                            exception_to_nan(
                                lambda z: mpmath.log(mpmath.ncdf(z))),
                            [Arg()], n=600, dps=300, rtol=1e-13)

    def test_log_ndtr_complex(self):
        assert_mpmath_equal(sc.log_ndtr,
                            exception_to_nan(lambda z: mpmath.log(
                                mpmath.erfc(-z/np.sqrt(2.))/2.)),
    ...
    def test_gammainc(self):
        ...
        assert_mpmath_equal(sc.gammainc,
                            lambda z, b: mpmath.gammainc(
                                z, b=b, regularized=True),
    ...
    def test_gammaincc(self):
        ...
        assert_mpmath_equal(sc.gammaincc,
                            lambda z, a: mpmath.gammainc(
                                z, a=a, regularized=True),
                            [Arg(0, 1e4, inclusive_a=False), Arg(0, 1e4)],
                            nan_ok=False, rtol=1e-11)
    ...
    def test_hankel2(self):
        assert_mpmath_equal(sc.hankel2,
                            exception_to_nan(
                                lambda v, x: mpmath.hankel2(v, x, **HYPERKW)),
                            [Arg(-1e20, 1e20), Arg()])
    ...
    def test_hyp0f1_complex(self):
        assert_mpmath_equal(lambda a, z: sc.hyp0f1(a.real, z),
                            exception_to_nan(
                                lambda a, x: mpmath.hyp0f1(a, x, **HYPERKW)),
    ...
    def test_hyp2f1_complex(self):
        ...
        assert_mpmath_equal(lambda a, b, c, x: sc.hyp2f1(a.real, b.real, c.real, x),
                            exception_to_nan(lambda a, b, c, x: mpmath.hyp2f1(
                                a, b, c, x, **HYPERKW)),
                            [Arg(-1e2, 1e2), Arg(-1e2, 1e2),
                             Arg(-1e2, 1e2), ComplexArg()],
                            n=10)
    ...
    def test_jacobi(self):
        assert_mpmath_equal(sc.eval_jacobi,
                            exception_to_nan(lambda a, b, c, x: mpmath.jacobi(
                                a, b, c, x, **HYPERKW)),
                            [Arg(), Arg(), Arg(), Arg()])
    ...
    def test_jacobi_int(self):
        ...
        assert_mpmath_equal(lambda n, a, b, x: sc.eval_jacobi(int(n), a, b, x),
                            lambda n, a, b, x: exception_to_nan(
                                jacobi)(n, a, b, x, **HYPERKW),
                            [IntArg(), Arg(), Arg(), Arg()],
                            n=20000, dps=50)
    ...
    def test_ker(self):
        assert_mpmath_equal(sc.ker,
                            exception_to_nan(
                                lambda x: mpmath.ker(0, x, **HYPERKW)),
                            [Arg(-1e30, 1e30)], n=1000)

    @nonfunctional_tooslow
    def test_laguerre(self):
        assert_mpmath_equal(trace_args(sc.eval_laguerre),
                            lambda n, x: exception_to_nan(
                                mpmath.laguerre)(n, x, **HYPERKW),
                            [Arg(), Arg()])
    ...
    def test_legenp_complex_2(self):
        ...
        assert_mpmath_equal(clpnm,
                            ...
                            [FixedArg([-2, -1, 0, 1, 2, 10]),
                             FixedArg([-2, -1, 0, 1, 2, 10]), FixedArg(z)],
                            rtol=1e-6,
                            n=500)
    ...
    def test_pcfd(self):
        ...
        assert_mpmath_equal(pcfd,
                            exception_to_nan(
                                lambda v, x: mpmath.pcfd(v, x, **HYPERKW)),
                            [Arg(), Arg()])
    ...
    def test_shi(self):
        ...
        # check asymptotic series cross-over
        assert_mpmath_equal(
            shi, mpmath.shi, [FixedArg([88 - 1e-9, 88, 88 + 1e-9])])

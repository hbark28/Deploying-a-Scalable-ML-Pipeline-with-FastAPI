...
def test_aliasing2():
    ...
    assert_allclose(lp.A_ub, lp_copy.A_ub,
                    err_msg="A_ub modified by _clean_inputs")
    assert_allclose(lp.b_ub, lp_copy.b_ub,
                    err_msg="b_ub modified by _clean_inputs")
    assert_allclose(lp.A_eq, lp_copy.A_eq,
                    err_msg="A_eq modified by _clean_inputs")
    assert_allclose(lp.b_eq, lp_copy.b_eq,
...
def test_too_many_dimensions():
    ...
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=bad2D, A_ub=A, b_ub=cb))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_ub=bad3D, b_ub=cb))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_ub=A, b_ub=bad2D))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_eq=bad3D, b_eq=cb))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_eq=A, b_eq=bad2D))
...
def test_too_few_dimensions():
    ...
    cb = np.random.rand(4)
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_ub=bad, b_ub=cb))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=cb, A_eq=bad, b_eq=cb))
...
def test_inconsistent_dimensions():
    ...
    boundsbad = [(0, 1)] * (n + 1)
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=c, A_ub=Abad, b_ub=bgood))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=c, A_ub=Agood, b_ub=bbad))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=c, A_eq=Abad, b_eq=bgood))
    assert_raises(ValueError, _clean_inputs,
                  _LPProblem(c=c, A_eq=Agood, b_eq=bbad))
    assert_raises(ValueError, _clean_inputs, _LPProblem(c=c, bounds=boundsbad))
    assert_raises(ValueError, _clean_inputs, _LPProblem(
        c=c, bounds=[[1, 2], [2, 3], [3, 4], [4, 5, 6]]))
...
def test_type_errors():
    ...
    assert_raises(ValueError, _clean_inputs,
                  lp._replace(bounds=[(1, 2), (1, "")]))
    assert_raises(TypeError, _clean_inputs, lp._replace(
        bounds=[(1, date(2020, 2, 29))]))
...
def test_bad_bounds():
    ...
    assert_raises(ValueError, _clean_inputs, lp._replace(
        bounds=[(1, 2), (1, 2), (1, 2)]))
...
def test_good_bounds():
    ...
    lp_cleaned = _clean_inputs(lp._replace(
        bounds=[(None, None), (-np.inf, None)]))

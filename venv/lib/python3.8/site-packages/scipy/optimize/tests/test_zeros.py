...
class TestBasic:
    ...
    def test_root_scalar_fail(self):
        ...
        with pytest.raises(ValueError):
            root_scalar(f1, method='halley', fprime=f1_1,
                        x0=3, xtol=1e-6)  # no fprime2
        with pytest.raises(ValueError):
            root_scalar(f1, method='halley', fprime2=f1_2,
                        x0=3, xtol=1e-6)  # no fprime
    ...
    def test_newton_combined(self):
        ...
        def f1_1(x): return 2*x - 2
        def f1_2(x): return 2.0 + 0*x
        ...
        sol0 = root_scalar(f1, method='halley', x0=3,
                           fprime=f1_1, fprime2=f1_2)
    ...
    def test_newton_full_output(self):
        ...
        for derivs in range(3):
            ...
            x, r = zeros.newton(self.f1, x0, maxiter=iters,
                                disp=False, **kwargs)
    ...
    def test_deriv_zero_warning(self):
        def func(x): return x**2 - 2.0
        def dfunc(x): return 2*x
        assert_warns(RuntimeWarning, zeros.newton,
                     func, 0.0, dfunc, disp=False)

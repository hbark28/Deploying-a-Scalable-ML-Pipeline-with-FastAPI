...
def test_performance():
    ...
    for xscale, yscale, line_search in itertools.product([1.0, 1e-10, 1e10], [1.0, 1e-10, 1e10],
        ...
        for problem in table_1:
            n = problem['n']
            def func(x, n): return yscale*problem['F'](x/xscale, n)
            args = (n,)
            x0 = problem['x0'](n) * xscale

            fatol = np.sqrt(n) * e_a * yscale + e_r * \
                np.linalg.norm(func(x0, n))
...
def F_4(x, n):
    ...
    g[::3] = 0.6 * x[::3] + 1.6 * x[1::3]**3 - \
        7.2 * x[1::3]**2 + 9.6 * x[1::3] - 4.8
    g[1::3] = 0.48 * x[::3] - 0.72 * x[1::3]**3 + 3.24 * \
        x[1::3]**2 - 4.32 * x[1::3] - x[2::3] + 0.2 * x[2::3]**3 + 2.16

...
class TestFSolve:
    ...
    def test_wrong_shape_fprime_callable(self):
        ...
        assert_raises(TypeError, optimize.fsolve, func,
                      x0=[0, 1], fprime=deriv_func)

    def test_wrong_shape_fprime_function(self):
        def func(x): return dummy_func(x, (2,))
        def deriv_func(x): return dummy_func(x, (3, 3))
        assert_raises(TypeError, optimize.fsolve, func,
                      x0=[0, 1], fprime=deriv_func)
    ...
    def test_Dfun_can_raise(self):
        def func(x): return x - np.array([10])
    ...
    def test_float32(self):
        def func(x): return np.array(
            [x[0] - 100, x[1] - 1000], dtype=np.float32)**2
...
class TestLeastSq:
    ...
    def test_wrong_shape_func_function(self):
        ...
        # length 1, so this should result in a TypeError.
        x0 = [1.5, 2.0]
        assert_raises(TypeError, optimize.leastsq,
                      dummy_func, x0, args=((1,),))

    def test_wrong_shape_Dfun_callable(self):
        func = ReturnShape(1)
        deriv_func = ReturnShape((2, 2))
        assert_raises(TypeError, optimize.leastsq,
                      func, x0=[0, 1], Dfun=deriv_func)
...
class TestCurveFit:
    ...
    def test_pcov(self):
        ...
        with suppress_warnings() as sup:
            ...
            popt, pcov = curve_fit(f_flat, xdata, ydata,
                                   p0=[2, 0], sigma=sigma)
    ...
    def test_maxfev_and_bounds(self):
        ...
        popt2, _ = curve_fit(lambda x, p: p*x, x, y,
                             bounds=(0, 3), max_nfev=100)
    ...
    def test_broadcast_y(self):
        ...
        def fit_func(x, a, b): return a*x**2 + b*x - target
        for method in ['lm', 'trf', 'dogbox']:

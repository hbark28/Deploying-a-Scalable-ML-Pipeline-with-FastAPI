...
class TestMultivariateNormal:
    ...
    def test_frozen(self):
        ...
        assert_allclose(norm_frozen.pdf(
            x), multivariate_normal.pdf(x, mean, cov))
        assert_allclose(norm_frozen.logpdf(x),
                        multivariate_normal.logpdf(x, mean, cov))
        assert_allclose(norm_frozen.cdf(
            x), multivariate_normal.cdf(x, mean, cov))
...
class TestMatrixNormal:
    ...
    def test_array_input(self):
        ...
        X = np.concatenate(
            (X1[np.newaxis, :, :, :], X2[np.newaxis, :, :, :]), axis=0)
...
class TestSpecialOrthoGroup:
    ...
    def test_haar(self):
        ...
        proj = dict(((er, ec), sorted([x[er][ec]
                    for x in xs])) for er, ec in els)
...
class TestOrthoGroup:
    ...
    def test_det_and_ortho(self):
        ...
        assert_array_less([0]*10, [np.nonzero(d < 0)[0].shape[0]
                          for d in dets])
        assert_array_less([np.nonzero(d < 0)[0].shape[0]
                          for d in dets], [10]*10)
...
class TestUnitaryGroup:
    def test_reproducibility(self):
        ...
        expected = np.array([[0.308771+0.360312j, 0.044021+0.622082j, 0.160327+0.600173j],
                             [0.732757+0.297107j, 0.076692 -
                                 0.4614j, -0.394349+0.022613j],
    ...
    def test_unitarity(self):
        ...
        for x in xs:
            assert_allclose(np.dot(x, x.conj().T),
                            np.eye(x.shape[0]), atol=1e-15)
...
class TestMultivariateT:
    ...
    def test_allow_singular(self):
        ...
        args = dict(loc=[0, 0], shape=[[0, 0], [0, 1]],
                    df=1, allow_singular=False)

...
class TestShapiro:
    def test_basic(self):
        ...
        assert_almost_equal(shapiro_test.statistic,
                            0.90047299861907959, decimal=6)
        assert_almost_equal(pw, 0.042089745402336121, decimal=6)
        assert_almost_equal(shapiro_test.pvalue,
                            0.042089745402336121, decimal=6)
        ...
        assert_almost_equal(shapiro_test.statistic,
                            0.9772805571556091, decimal=6)
        assert_almost_equal(pw, 0.08144091814756393, decimal=3)
        assert_almost_equal(shapiro_test.pvalue,
                            0.08144091814756393, decimal=3)
...
class TestBinomTestP:
    ...
    def test_boost_overflow_raises(self):
        ...
        assert_raises(OverflowError, self.binom_test_func,
                      5.0, 6, p=sys.float_info.min)
...
class TestFligner:
    ...
    def test_trimmed1(self):
        ...
        def _perturb(g): return (np.asarray(g) +
                                 1e-10*rs.randn(len(g))).tolist()
        g1_ = _perturb(g1)
        g2_ = _perturb(g2)
        g3_ = _perturb(g3)
        # Test that center='trimmed' gives the same result as center='mean'
...
class TestWilcoxon:
    ...
    def test_accuracy_wilcoxon(self):
        ...
        x = np.array([120, 114, 181, 188, 180, 146,
                     121, 191, 132, 113, 127, 112])
        y = np.array([133, 143, 119, 189, 112, 199,
                     198, 113, 115, 121, 142, 187])

...
def pickle_function(func):
    ...
    if test is not func:
        raise pickle.PicklingError(
            "Can't pickle {}: it's not the same object as {}".format(
                func, test)
        )
...
def determine_backend_multi(
    ...
    if "dispatch_type" in kwargs:
        ...
        if not all(isinstance(d, Dispatchable) for d in dispatchables):
            raise TypeError(
                "dispatchables must be instances of uarray.Dispatchable")

    if len(kwargs) != 0:
        raise TypeError(
            "Received unexpected keyword arguments: {}".format(kwargs))

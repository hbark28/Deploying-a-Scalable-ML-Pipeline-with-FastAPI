...
class TestSmoothBivariateSpline:
    def test_linear_constant(self):
        ...
        assert_array_almost_equal(
            lut.get_knots(), ([1, 1, 3, 3], [1, 1, 3, 3]))
        assert_array_almost_equal(lut.get_coeffs(), [3, 3, 3, 3])
        assert_almost_equal(lut.get_residual(), 0.0)
        assert_array_almost_equal(lut([1, 1.5, 2], [1, 1.5]), [
                                  [3, 3], [3, 3], [3, 3]])
...
class TestLSQSphereBivariateSpline:
    ...
    def test_empty_input(self):
        ...
        assert_array_almost_equal(self.lut_lsq(
            [], [], grid=False), np.zeros((0,)))
...
class TestRectBivariateSpline:
    def test_defaults(self):
        x = array([1, 2, 3, 4, 5])
        y = array([1, 2, 3, 4, 5])
        z = array([[1, 2, 1, 2, 1], [1, 2, 1, 2, 1], [
                  1, 2, 3, 2, 1], [1, 2, 2, 2, 1], [1, 2, 1, 2, 1]])
...
class TestRectSphereBivariateSpline:
    ...
    def test_derivatives(self):
        ...
        assert_allclose(lut(x, y, dtheta=1, grid=False),
                        _numdiff_2d(lambda x, y: lut(
                            x, y, grid=False), x, y, dx=1),
                        rtol=1e-4, atol=1e-4)
        assert_allclose(lut(x, y, dphi=1, grid=False),
                        _numdiff_2d(lambda x, y: lut(
                            x, y, grid=False), x, y, dy=1),
                        rtol=1e-4, atol=1e-4)
        assert_allclose(lut(x, y, dtheta=1, dphi=1, grid=False),
                        _numdiff_2d(lambda x, y: lut(
                            x, y, grid=False), x, y, dx=1, dy=1, eps=1e-6),
                        rtol=1e-3, atol=1e-3)

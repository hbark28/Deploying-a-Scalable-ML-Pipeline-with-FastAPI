...
class SchemaTransformer:
    ...
    get_json_schema: Callable[[CoreSchema,
...
def get_enum_core_schema(enum_type: type[Enum], config: ConfigDict) -> CoreSchema:
    ...
    def get_json_schema(_, handler: GetJsonSchemaHandler) -> JsonSchemaValue:
        json_schema = handler(core_schema.literal_schema(
            [x.value for x in cases], ref=enum_ref))
    ...
    if len(cases) == 1:
        ...
        expected = ', '.join([repr(case.value)
                             for case in cases[:-1]]) + f' or {cases[-1].value!r}'
    ...
    def to_enum(__input_value: Any) -> Enum:
        try:
            ...
            raise PydanticCustomError('enum', f'Input should be {expected}', {
    ...
    if use_enum_values:
        strict_python_schema = core_schema.chain_schema(
            [strict_python_schema, core_schema.no_info_plain_validator_function(
                lambda x: x.value)]
        )
    ...
    return core_schema.lax_or_strict_schema(
        lax_schema=lax, strict_schema=strict, ref=enum_ref, metadata={
...
def decimal_prepare_pydantic_annotations(
    ...
    _known_annotated_metadata.check_metadata(
        metadata, {*_known_annotated_metadata.FLOAT_CONSTRAINTS,
                   'max_digits', 'decimal_places'}, decimal.Decimal
    )
    ...
    return (
        ...
        [
            InnerSchemaValidator(
                schema, js_core_schema=constrained_str_schema, js_schema_update={'format': 'path'}),
...
class SequenceValidator:
    ...
    def __get_pydantic_core_schema__(self, source_type: Any, handler: GetCoreSchemaHandler) -> CoreSchema:
        ...
        metadata = {'min_length': self.min_length,
                    'max_length': self.max_length, 'strict': self.strict}
        ...
        if self.mapped_origin in (list, set, frozenset):
            ...
            if self.mapped_origin is collections.deque:
                ...
                coerce_instance_wrap = partial(
                    ...
                    partial(dequeue_validator, maxlen=metadata.get(
                        'max_length', None)),
                )
            else:
                coerce_instance_wrap = partial(
...
def sequence_like_prepare_pydantic_annotations(
    ...
    mapped_origin = SEQUENCE_ORIGIN_MAP.get(
        origin, None) if origin else SEQUENCE_ORIGIN_MAP.get(source_type, None)
...
def get_defaultdict_default_default_factory(values_source_type: Any) -> Callable[[], Any]:
    def infer_default() -> Callable[[], Any]:
        ...
        values_type_origin = get_origin(
            values_source_type) or values_source_type
...
def mapping_like_prepare_pydantic_annotations(
    ...
    mapped_origin = MAPPING_ORIGIN_MAP.get(
        origin, None) if origin else MAPPING_ORIGIN_MAP.get(source_type, None)
    if mapped_origin is None:
        return None
    ...
    return (
        source_type,
        [
            MappingValidator(mapped_origin, keys_source_type,
                             values_source_type, **metadata),
            *remaining_annotations,
        ],
    )

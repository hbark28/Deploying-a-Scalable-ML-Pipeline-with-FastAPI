...
FROZEN_SET_TYPES: list[type] = [frozenset,
                                typing.FrozenSet, collections.abc.Set]
...
class TypesNamespaceStack:
    ...
    def push(self, for_type: type[Any]):
        types_namespace = {
            **_typing_extra.get_cls_types_namespace(for_type), **(self.tail or {})}
...
class GenerateSchema:
    ...
    def _common_field_schema(  # C901
        ...
        if has_instance_in_type(field_info.annotation, (ForwardRef, str)):
            ...
            if evaluated is not field_info.annotation and not has_instance_in_type(evaluated, PydanticRecursiveRef):
                ...
                field_info.metadata = [
                    *new_field_info.metadata, *field_info.metadata]
        ...
        schema = apply_validators(schema, filter_field_decorator_info_by_field(
            this_field_validators, name), name)
        schema = apply_validators(
            schema, filter_field_decorator_info_by_field(
                decorators.field_validators.values(), name), name
        )
        ...
        metadata = build_metadata_dict(
            js_annotation_functions=[get_json_schema_update_func(
    ...
    def _typed_dict_schema(self, typed_dict_cls: Any, origin: Any) -> core_schema.CoreSchema:
        ...
        with self.defs.get_schema_or_ref(typed_dict_cls) as (typed_dict_ref, maybe_schema):
            ...
            with self._config_wrapper_stack.push(config), self._types_namespace_stack.push(typed_dict_cls):
                ...
                self.defs.definitions[typed_dict_ref] = self._post_process_generated_schema(
                    schema)
        ...
        with self.defs.get_schema_or_ref(dataclass) as (dataclass_ref, maybe_schema):
            ...
            with self._config_wrapper_stack.push(config), self._types_namespace_stack.push(dataclass):
                ...
                decorators = dataclass.__dict__.get(
                    '__pydantic_decorators__') or DecoratorInfos.build(dataclass)
                ...
                schema = apply_model_validators(
                    schema, model_validators, 'outer')
                self.defs.definitions[dataclass_ref] = self._post_process_generated_schema(
                    schema)
    ...
    def _computed_field_schema(
        ...
        metadata = build_metadata_dict(js_annotation_functions=[
                                       set_computed_field_metadata])
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[[Callable[..., Any], core_schema.CoreSchema,
              str | None], core_schema.CoreSchema],
] = {

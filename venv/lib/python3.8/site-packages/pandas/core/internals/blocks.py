...
class Block(PandasObject):
    ...
    def to_native_types(self, na_rep: str = "nan", quoting=None, **kwargs) -> Block:
        ...
        result = to_native_types(
            self.values, na_rep=na_rep, quoting=quoting, **kwargs)
            ...
            for blk_num, blk in enumerate(rb):
                ...
                if convert and blk.is_object and not all(x is None for x in dest_list):
                    ...
                    result = extend_blocks(
                        [
                            b.convert(copy=True and not using_cow,
                                      using_cow=using_cow)
                            for b in result
                        ]
                    )
            ...
            if value is None:
                ...
                if mask.any():
                    ...
                    nb = self.astype(np.dtype(object),
                                     copy=False, using_cow=using_cow)
    ...
    def fillna(
        ...
        if noop:
            ...
            if inplace:
                ...
                nbs = nb._maybe_downcast(
                    [nb], downcast=downcast, using_cow=using_cow)
                return nbs
        ...
        return extend_blocks(
            [
                blk._maybe_downcast(
                    [blk], downcast=downcast, using_cow=using_cow)
                for blk in nbs
            ]
        )
        ...
        **kwargs,
        ...
        if values.ndim == 2 and axis == 0:
            ...
            new_values = values.T.fillna(
                value=fill_value, method=method, limit=limit).T
        else:

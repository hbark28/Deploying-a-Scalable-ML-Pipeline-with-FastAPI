...
def dict_to_mgr(
    ...
    *,
    ...
    if columns is not None:
        ...
        arrays = [arr if not isinstance(
            arr, Index) else arr._data for arr in arrays]

    if copy:
        if typ == "block":
            # We only need to copy arrays that will not get consolidated, i.e.
            #  only EA arrays
            arrays = [x.copy() if isinstance(x, ExtensionArray)
                      else x for x in arrays]
        else:
...
def to_arrays(
    ...
    if isinstance(data, ABCDataFrame):
        ...
        if columns is not None:
            ...
            arrays = [
                data._ixs(i, axis=1)._values for i in range(len(columns))]
            ...
            if dtype is None:
                if arr.dtype == np.dtype("O"):
                    ...
                    if is_integer_dtype(arr.dtype):
                        arr = IntegerArray(arr, np.zeros(
                            arr.shape, dtype=np.bool_))
                    elif is_bool_dtype(arr.dtype):
                        arr = BooleanArray(arr, np.zeros(
                            arr.shape, dtype=np.bool_))

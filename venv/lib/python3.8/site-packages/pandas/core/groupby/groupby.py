    ...
    **Attributes**
    ...
    def _set_result_index_ordered(
        ...
        result = result.set_axis(
            original_positions, axis=self.axis, copy=False)
    ...
    def _agg_py_fallback(
        ...
        if isinstance(values, Categorical):
            ...
            res_values = type(values)._from_sequence(
                res_values, dtype=values.dtype)
    ...
    def sem(self, ddof: int = 1, numeric_only: bool = False):
        ...
        if result.ndim == 1:
            ...
            result.iloc[:,
                        result_ilocs] /= np.sqrt(counts.iloc[:, count_ilocs])
        return result
    ...
    def _fill(self, direction: Literal["ffill", "bfill"], limit=None):
        ...
        sorted_labels = np.argsort(
            ids, kind="mergesort").astype(np.intp, copy=False)
        if direction == "bfill":
    ...
    def shift(self, periods: int = 1, freq=None, axis: Axis = 0, fill_value=None):
        ...
        if freq is not None or axis != 0:
            def f(x): return x.shift(periods, freq, axis, fill_value)
            return self._python_apply_general(f, self._selected_obj, is_transform=True)
    ...
    def diff(self, periods: int = 1, axis: AxisInt = 0) -> NDFrameT:
        ...
        if obj.ndim == 1:
            ...
            to_coerce = [c for c, dtype in obj.dtypes.items()
                         if dtype in dtypes_to_f32]
            if len(to_coerce):
    ...
    def sample(
        ...
        group_iterator = self.grouper.get_iterator(

...
class TestMerge:
    ...
    def test_merge_index_singlekey_inner(self):
        ...
        # inner join
        result = merge(left, right, left_on="key",
                       right_index=True, how="inner")
        expected = left.join(right, on="key").loc[result.index]
        tm.assert_frame_equal(result, expected)

        result = merge(right, left, right_on="key",
    ...
    def test_handle_join_key_pass_array(self):
        ...
        expected = Series([1, 1, 1, 1, 2, 2, 3, 4, 5],
                          dtype=np.int_, name="key_0")
    ...
    def test_merge_left_empty_right_empty(self, join_type, kwarg):
        ...
        exp_in = DataFrame(
            columns=["a", "b", "c", "x", "y", "z"], dtype=object)
    ...
    def test_merge_datetime64tz_with_dst_transition(self):
        # GH 18885
        df1 = DataFrame(
            pd.date_range("2017-10-29 01:00", periods=4,
    ...
    def test_validation(self):
        ...
        result = merge(left, right, left_index=True,
...
class TestMergeDtypes:
    ...
    def test_merge_ea_with_string(self, join_type, string_dtype):
        ...
        df2_copy = df2.copy()
        merged = merge(left=df1, right=df2, on=[
    ...
    def test_merge_empty(self, left_empty, how, exp):
        ...
        if exp == "left":
            ...
            expected = DataFrame(
                columns=["A_x", "B", "A_y", "C"], dtype="int64")
...
def test_merge_multiindex_columns():
    ...
    numbers = ["1", "2", "3"]
    index = MultiIndex.from_product(
        (letters, numbers), names=["outer", "inner"])
...
def test_categorical_non_unique_monotonic(n_categories):
    ...
    df1 = DataFrame(range(n_categories + 1),
                    columns=["value"], index=left_index)
    df2 = DataFrame(
        [[6]],
        columns=["value"],

...
class TestJoin:
    ...
    def test_join_multiindex(self):
        ...
        df1 = DataFrame(data=np.random.randn(
            6), index=index1, columns=["var X"])
        df2 = DataFrame(data=np.random.randn(
            6), index=index2, columns=["var Y"])
    ...
    def test_join_float64_float32(self):
        a = DataFrame(np.random.randn(10, 2), columns=[
                      "a", "b"], dtype=np.float64)
    ...
    def test_mixed_type_join_with_suffix(self):
        # GH #916
        df = DataFrame(np.random.randn(20, 6), columns=[
                       "a", "b", "c", "d", "e", "f"])
        df.insert(0, "id", 0)
        df.insert(5, "dt", "foo")
    ...
    def test_join_dups(self):
        ...
        df = concat(
            [
                DataFrame(np.random.randn(10, 4),
                          columns=["A", "A", "B", "B"]),
                DataFrame(

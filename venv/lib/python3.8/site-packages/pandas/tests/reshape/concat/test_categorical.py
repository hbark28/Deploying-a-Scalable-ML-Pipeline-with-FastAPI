...
class TestCategoricalConcat:
    def test_categorical_concat(self, sort):
        ...
        df2 = DataFrame(np.arange(14, dtype="int64").reshape(
            7, 2), columns=["a", "c"])
    ...
    def test_categorical_concat_preserve(self):
        ...
        exp = Series(list("abcabc"), index=[
                     0, 1, 2, 0, 1, 2], dtype="category")
        res = pd.concat([s, s])
        tm.assert_series_equal(res, exp)
    ...
    def test_categorical_index_upcast(self):
        ...
        res = pd.concat([a, b])
        exp = DataFrame({"foo": [1, 2, 4, 3]}, index=[
                        "foo", "bar", "baz", "bar"])
    ...
    def test_categorical_missing_from_one_frame(self):
        ...
        df2 = DataFrame(
            {"f1": [2, 3, 1], "f2": Series([4, 4, 4]).astype("category")})
    ...
    def test_concat_categorical_same_categories_different_order(self):
        ...
        c1 = pd.CategoricalIndex(["a", "a"], categories=[
                                 "a", "b"], ordered=False)
        c2 = pd.CategoricalIndex(["b", "b"], categories=[
                                 "b", "a"], ordered=False)

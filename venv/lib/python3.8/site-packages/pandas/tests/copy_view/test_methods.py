...
def test_copy_shallow(using_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        assert not np.shares_memory(
            get_array(df_copy, "a"), get_array(df, "a"))
...
def test_methods_copy_keyword(
    ...
    if "to_timestamp" in request.node.callspec.id:
        ...
        index = date_range("2012-01-01", freq="D",
...
def test_shift_columns(using_copy_on_write):
    ...
    assert np.shares_memory(get_array(df2, "2020-01-02"),
                            get_array(df, "2020-01-01"))
    df.iloc[0, 1] = 0
    if using_copy_on_write:
        assert not np.shares_memory(
...
def test_pop(using_copy_on_write):
    ...
    if using_copy_on_write:
        result.iloc[0] = 0
        assert not np.shares_memory(
            result.values, get_array(view_original, "a"))
    df.iloc[0, 0] = 0
    if using_copy_on_write:
        assert not np.shares_memory(
...
def test_round(using_copy_on_write, decimals):
    ...
    if using_copy_on_write:
        ...
        if decimals >= 0:
            ...
            assert not np.shares_memory(
                get_array(df2, "a"), get_array(df, "a"))
    else:
        assert not np.shares_memory(get_array(df2, "b"), get_array(df, "b"))
...
def test_putmask_dont_copy_some_blocks(using_copy_on_write, val, exp):
    ...
    if using_copy_on_write:
        ...
        assert np.shares_memory(get_array(view, "b"),
                                get_array(df, "b")) is exp

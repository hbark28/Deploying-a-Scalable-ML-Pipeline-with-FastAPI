...
class TestDataFrameEval:
    ...
    def test_eval_resolvers_as_list(self):
        ...
        dict2 = {"b": 2}
        assert df.eval(
            "a + b", resolvers=[dict1, dict2]) == dict1["a"] + dict2["b"]
        assert pd.eval(
            "a + b", resolvers=[dict1, dict2]) == dict1["a"] + dict2["b"]
...
class TestDataFrameQueryNumExprPandas:
    ...
    def test_date_query_no_attribute_access(self):
        ...
        df["dates2"] = date_range("1/1/2013", periods=5)
        df["dates3"] = date_range("1/1/2014", periods=5)
        res = df.query("dates1 < 20130101 < dates3",
    ...
    def test_date_index_query(self):
        ...
        res = df.query("index < 20130101 < dates3",
                       engine=engine, parser=parser)
    ...
    def test_nested_scope(self):
        ...
        result = pd.eval("df[df > 0 and df2 > 0]",
                         engine=engine, parser=parser)
        tm.assert_frame_equal(result, expected)
    ...
    def test_chained_cmp_and_in(self):
        ...
        ind = (df.a < df.b) & (df.b < df.c) & ~df.b.isin(
            df.a) & ~df.c.isin(df.b)
        expec = df[ind]
        tm.assert_frame_equal(res, expec)
    ...
    def test_method_calls_in_query(self):
        ...
        result = df.query("a.notnull()", engine=self.engine,
                          parser=self.parser)
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameQueryStrings:
    ...
    def test_str_list_query_method(self, parser, engine):
        ...
        if parser != "pandas":
            ...
            res = df.query('strings == ["a", "b"]',
                           engine=engine, parser=parser)
            tm.assert_frame_equal(res, expect)

            res = df.query('["a", "b"] == strings',
                           engine=engine, parser=parser)
            tm.assert_frame_equal(res, expect)
    ...
    def test_query_with_nested_strings(self, parser, engine):
        ...
        res = df.query("""'"page 1 load"' in event""",
                       parser=parser, engine=engine)

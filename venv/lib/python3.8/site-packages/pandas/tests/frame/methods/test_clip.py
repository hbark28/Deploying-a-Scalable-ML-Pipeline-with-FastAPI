...
class TestDataFrameClip:
    ...
    def test_inplace_clip(self, float_frame):
        ...
        return_value = frame_copy.clip(
            upper=median, lower=median, inplace=True)
    ...
    def test_clip_against_list_like(self, simple_frame, inplace, lower, axis, res):
        ...
        result = original.clip(lower=lower, upper=[
                               5, 6, 7], axis=axis, inplace=inplace)

        expected = DataFrame(res, columns=original.columns,
    ...
    def test_clip_against_unordered_columns(self):
        ...
        expected_lower_upper = df1.clip(
            lower=df3[df1.columns], upper=df2[df1.columns])
    ...
    def test_clip_with_na_args(self, float_frame):
        ...
        tm.assert_frame_equal(float_frame.clip(
            upper=np.nan, lower=np.nan), float_frame)

        # GH#19992 and adjusted in GH#40420
        df = DataFrame({"col_0": [1, 2, 3], "col_1": [
                       4, 5, 6], "col_2": [7, 8, 9]})
        ...
        expected = DataFrame(
            {"col_0": [9, -3, 0, 6, 5], "col_1": [2, -4, 6, 8, 3]})
        tm.assert_frame_equal(result, expected)

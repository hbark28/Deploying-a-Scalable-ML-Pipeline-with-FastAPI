...
class TestReindexSetIndex:
    ...
    def test_reindex_timestamp_with_fold(self, timezone, year, month, day, hour):
        ...
        tm.assert_frame_equal(
            df,
            DataFrame({"index": ["1", "2"], "vals": [
                      None, None]}).set_index("index"),
        )
    ...
    def test_reindex_with_categoricalindex(self):
        df = DataFrame(
            ...
            index=CategoricalIndex(
                list("abc"), dtype=CDT(list("cabe")), name="B"),
        )
        ...
        expected = DataFrame(
            {"A": [0, np.nan], "B": Series(
                list("ae")).astype(CDT(cats, ordered=True))}
        ).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
        ...
        df2 = DataFrame(
            ...
            index=CategoricalIndex(
                list("aabbca"), dtype=CDT(list("cabe")), name="B"),
        )

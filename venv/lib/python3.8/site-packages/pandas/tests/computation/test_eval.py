...
def should_warn(*args):
    ...
    only_one_dt = reduce(
        operator.xor, map(lambda x: issubclass(
            x.dtype.type, np.datetime64), args)
    )
...
class TestAlignment:
    ...
    def test_frame_comparison(self, engine, parser, r_idx_type, c_idx_type):
        ...
        df3 = DataFrame(np.random.randn(*df.shape),
...
class TestOperations:
    ...
    def test_basic_period_index_boolean_expression(self):
        df = tm.makeCustomDataframe(
            2, 2, data_gen_f=f, c_idx_type="p", r_idx_type="i")
...
def test_eval_no_support_column_name(request, column):
    ...
    df = DataFrame(np.random.randint(0, 100, size=(10, 2)),
                   columns=[column, "col1"])

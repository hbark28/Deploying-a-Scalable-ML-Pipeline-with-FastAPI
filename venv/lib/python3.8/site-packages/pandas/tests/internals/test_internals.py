...
def assert_block_equal(left, right):
    ...
    tm.assert_numpy_array_equal(
        left.mgr_locs.as_array, right.mgr_locs.as_array)
    ...
    def test_copy(self, mgr):
        ...
        for blk, cp_blk in zip(mgr.blocks, cp.blocks):
            ...
            if isinstance(cpvals, DatetimeArray):
                assert (lbase is None and rbase is None) or (
                    lbase is not rbase)
    ...
    def test_get_slice(self, mgr):
        ...
        for ax in range(mgr.ndim):
            ...
            if mgr.ndim < 2:
                ...
                assert_slice_ok(mgr, ax, np.ones(
                    mgr.shape[ax], dtype=np.bool_))
                assert_slice_ok(mgr, ax, np.zeros(
                    mgr.shape[ax], dtype=np.bool_))
    ...
    def test_take(self, mgr):
        def assert_take_ok(mgr, axis, indexer):
            ...
            tm.assert_index_equal(
                mgr.axes[axis].take(indexer), taken.axes[axis])
...
def test_make_block_no_pandas_array(block_maker):
    ...
    if block_maker is make_block:
        ...
        result = block_maker(arr, slice(len(arr)),
                             dtype=arr.dtype, ndim=arr.ndim)
        assert result.dtype.kind in ["i", "u"]
        assert result.is_extension is False

...
def test_apply(datetime_series):
    with np.errstate(all="ignore"):
        tm.assert_series_equal(datetime_series.apply(
            np.sqrt), np.sqrt(datetime_series))

        # element-wise apply
        tm.assert_series_equal(datetime_series.apply(
            math.exp), np.exp(datetime_series))
...
def test_series_map_stringdtype(any_string_dtype):
    ...
    expected = Series(data=["rabbit", "dog", "cat",
                      pd.NA], dtype=any_string_dtype)
...
def test_apply_empty_integer_series_with_datetime_index():
    # GH 21245
    s = Series([], index=pd.date_range(
        start="2018-01-01", periods=0), dtype=int)
...
def test_apply_retains_column_name():
    ...
    result = df.x.apply(lambda x: Series(
        range(x + 1), Index(range(x + 1), name="y")))
    expected = DataFrame(

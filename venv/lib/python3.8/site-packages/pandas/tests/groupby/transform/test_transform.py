...
def test_transform_axis_1(request, transformation_func):
    ...
    result = df.groupby([0, 0, 1], axis=1).transform(
...
def test_cython_transform_series(op, args, targop):
    ...
    for data in [s, s_missing]:
        ...
        tm.assert_series_equal(
            expected, data.groupby(labels).transform(op, *args))
...
def test_cython_transform_frame(op, args, targop):
    ...
    for df in [df, df2]:
        for gb_target in [
            ...
            result = gb[expected.columns].transform(
                op, *args).sort_index(axis=1)
            tm.assert_frame_equal(result, expected)
            result = getattr(gb[expected.columns], op)(
                *args).sort_index(axis=1)
            tm.assert_frame_equal(result, expected)
            # individual columns
            for c in df:
                if (
                    ...
                    expected.name = c
                    tm.assert_series_equal(
                        expected, gb[c].transform(op, *args))
...
def test_groupby_transform_rename():
    ...
    def demean_rename(x):
        ...
        result = result.rename(
            columns={c: f"{c}_demeaned" for c in result.columns})

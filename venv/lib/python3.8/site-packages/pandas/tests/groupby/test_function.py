...
@pytest.fixture(
    ...
    ids=["np.int32", "np.int64", "np.float32",
         "np.float64", "Int64", "Float64"],
)
...
def test_groupby_cumprod():
    ...
    expected = df.groupby("key", group_keys=False)[
        "value"].apply(lambda x: x.cumprod())
    expected.name = "value"
...
def test_nlargest_and_smallest_noop(data, groups, dtype, method):
    ...
    expidx = np.array(groups, dtype=np.int_) if isinstance(
        groups, list) else groups
    expected = Series(data, index=MultiIndex.from_arrays(
        [expidx, ser.index]), name="a")
    tm.assert_series_equal(result, expected)
...
def test_cummin(dtypes_for_minmax):
    ...
    tm.assert_frame_equal(result, expected)
    result = df.groupby("A", group_keys=False).B.apply(
    ...
    expected = (
        df.groupby("A", group_keys=False).B.apply(
            lambda x: x.cummin()).to_frame()
    )
    tm.assert_frame_equal(result, expected, check_exact=True)
    ...
    tm.assert_frame_equal(result, expected)
    expected = (
        base_df.groupby("A", group_keys=False).B.apply(
            lambda x: x.cummin()).to_frame()
    )
    tm.assert_frame_equal(result, expected)

...
@pytest.mark.parametrize(
    "groupbyfunc", [lambda x: x.weekday(), [lambda x: x.month,
                                            lambda x: x.weekday()]]
)
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    ...
    expected_index = pd.date_range(
        "1/1/2012", freq="3T", periods=6, name="dti")
    expected_columns = MultiIndex.from_tuples(
        [
            ("A", "ohlc", "open"),
            ("A", "ohlc", "high"),
    ...
    expected_values = np.hstack(
        [df.resample("3T").A.ohlc(), non_ohlc_expected_values])
    expected = DataFrame(
        expected_values, columns=expected_columns, index=expected_index
    )
    tm.assert_frame_equal(result, expected)
...
def test_callable_result_dtype_series(keys, agg_index, input, dtype, method):
    ...
    expected_index = pd.RangeIndex(
        0, 1) if method == "transform" else agg_index
    expected = Series([df["c"].iloc[0]], index=expected_index,
                      name="c").astype(dtype)
    tm.assert_series_equal(result, expected)
...
def test_groupby_aggregate_empty_key_empty_return():
    ...
    expected = DataFrame(columns=MultiIndex(
        levels=[["b"], []], codes=[[], []]))
    tm.assert_frame_equal(result, expected)
    ...
    def test_agg_multiple_lambda(self):
        ...
        result2 = df.groupby(by="kind").agg(
            ...
            weight_min=pd.NamedAgg(
                column="weight", aggfunc=lambda x: np.min(x)),
        )
        tm.assert_frame_equal(result2, expected)
...
def test_groupby_combined_aggs_cat_cols(grp_col_dict, exp_data):
    ...
    expected_df = DataFrame(
        data=exp_data, columns=multi_index, index=cat_index)

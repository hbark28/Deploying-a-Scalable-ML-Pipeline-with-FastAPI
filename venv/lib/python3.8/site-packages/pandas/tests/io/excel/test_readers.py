...
class TestReaders:
    ...
    def test_usecols_excel_range_str_invalid(self, read_ext):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.read_excel("test1" + read_ext,
                          sheet_name="Sheet1", usecols="D:E1")
    ...
    def test_index_col_empty(self, read_ext):
        ...
        expected = DataFrame(
            columns=["D", "E", "F"],
            index=MultiIndex(levels=[[]] * 3, codes=[[]]
                             * 3, names=["A", "B", "C"]),
        )
    ...
    def test_sheet_name(self, request, read_ext, df_ref):
        ...
        df2 = pd.read_excel(filename + read_ext,
                            index_col=0, sheet_name=sheet_name)
    ...
    def test_read_from_s3_url(self, read_ext, s3_resource, s3so):
        ...
        with open("test1" + read_ext, "rb") as f:
            s3_resource.Bucket(
                "pandas-test").put_object(Key="test1" + read_ext, Body=f)
    ...
    def test_euro_decimal_format(self, read_ext):
        ...
        result = pd.read_excel("test_decimal" + read_ext,
                               decimal=",", skiprows=1)
        expected = DataFrame(
            [

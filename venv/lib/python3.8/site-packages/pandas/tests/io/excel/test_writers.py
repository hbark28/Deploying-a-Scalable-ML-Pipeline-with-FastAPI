...
@pytest.mark.parametrize(
    "ext",
    [
        pytest.param(".xlsx", marks=[td.skip_if_no(
            "openpyxl"), td.skip_if_no("xlrd")]),
        pytest.param(".xlsm", marks=[td.skip_if_no(
            "openpyxl"), td.skip_if_no("xlrd")]),
        pytest.param(
...
class TestRoundTrip:
    ...
    def test_read_excel_parse_dates(self, ext):
        ...
        df = DataFrame(
            {"col": [1, 2, 3], "date_strings": pd.date_range(
...
class TestExcelWriter:
    ...
    def test_roundtrip(self, frame, path):
        ...
        recons = pd.read_excel(path, sheet_name="test1",
    ...
    def test_roundtrip_indexlabels(self, merge_cells, frame, path):
        ...
        df.to_excel(path, "test1", index_label=[
                    "test"], merge_cells=merge_cells)
        with ExcelFile(path) as reader:
            recons = pd.read_excel(reader, sheet_name="test1", index_col=0).astype(
                np.int64
            )
    ...
    def test_to_excel_interval_no_labels(self, path):
        ...
        df = DataFrame(np.random.randint(-10, 10,
                       size=(20, 1)), dtype=np.int64)
        expected = df.copy()
    ...
    def test_to_excel_multiindex_nan_label(self, merge_cells, path):
        df = DataFrame({"A": [None, 2, 3], "B": [
                       10, 20, 30], "C": np.random.sample(3)})
        df = df.set_index(["A", "B"])
    ...
    def test_to_excel_multiindex_dates(self, merge_cells, tsframe, path):
        ...
        new_index = [tsframe.index, np.arange(
            len(tsframe.index), dtype=np.int64)]
    ...
    def test_to_excel_empty_multiindex(self, path):
        ...
        df = DataFrame([], index=MultiIndex.from_tuples(
            [], names=[0, 1]), columns=[2])
        df.to_excel(path, "test1")
    ...
    def test_excel_010_hemstring(
        ...
        def roundtrip(data, header=True, parser_hdr=0, index=True):
            data.to_excel(path, header=header,
    ...
    def test_comment_arg(self, path):
        ...
        result2 = pd.read_excel(
            path, sheet_name="test_c", comment="#", index_col=0)
    ...
    def test_comment_used(self, path):
        ...
        result = pd.read_excel(path, sheet_name="test_c",
                               comment="#", index_col=0)
        tm.assert_frame_equal(result, expected)
...
class TestExcelWriterEngineTests:
    @pytest.mark.parametrize(
        "klass,ext",
        [
            pytest.param(_XlsxWriter, ".xlsx",
                         marks=td.skip_if_no("xlsxwriter")),
            pytest.param(_OpenpyxlWriter, ".xlsx",

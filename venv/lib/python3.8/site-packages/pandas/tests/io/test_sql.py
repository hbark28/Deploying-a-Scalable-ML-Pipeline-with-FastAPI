...
def test_read_iris_query_chunksize(conn, request):
    conn = request.getfixturevalue(conn)
    iris_frame = concat(read_sql_query(
        "SELECT * FROM iris", conn, chunksize=7))
    check_iris_frame(iris_frame)
    iris_frame = concat(pd.read_sql("SELECT * FROM iris", conn, chunksize=7))
    check_iris_frame(iris_frame)
    iris_frame = concat(pd.read_sql(
        "SELECT * FROM iris where 0=1", conn, chunksize=7))
    assert iris_frame.shape == (0, 5)
    assert "SepalWidth" in iris_frame.columns
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_to_sql_append(self, test_frame1):
        assert sql.to_sql(test_frame1, "test_frame4",
                          self.conn, if_exists="fail") == 4

        # Add to table again
        assert (
            sql.to_sql(test_frame1, "test_frame4",
                       self.conn, if_exists="append") == 4
        )
    ...
    def test_roundtrip(self, test_frame1):
        ...
        result = sql.read_sql_query(
            "SELECT * FROM test_frame_roundtrip", con=self.conn)
    ...
    def test_to_sql_index_label(self, index_name, index_label, expected):
        ...
        sql.to_sql(temp_frame, "test_index_label",
                   self.conn, index_label=index_label)
    ...
    def test_get_schema_with_schema(self, test_frame1):
        ...
        create_sql = sql.get_schema(
            test_frame1, "test", con=self.conn, schema="pypi")
    ...
    def test_get_schema_keys(self, test_frame1):
        ...
        create_sql = sql.get_schema(
            test_frame1, "test", con=self.conn, keys=["A", "B"])
    ...
    def test_escaped_table_name(self):
        ...
        res = sql.read_sql_query(
            "SELECT * FROM `d1187b08-4943-4c8d-a7f6`", self.conn)
    ...
    def test_read_sql_duplicate_columns(self):
        ...
        result = pd.read_sql(
            "SELECT a, b, a +1 as a, c FROM test_table;", self.conn)
        expected = DataFrame(
    ...
    def test_sql_open_close(self, test_frame3):
        ...
        with tm.ensure_clean() as name:
            with closing(self.connect(name)) as conn:
                assert (
                    sql.to_sql(test_frame3, "test_frame3_legacy",
                               conn, index=False)
                    == 4
                )
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_datetime_with_timezone(self, request):
        ...
        if not hasattr(df, "DateColWithTz"):
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="no column with datetime with time zone")
            )
    ...
    def test_datetime_with_timezone_roundtrip(self):
        ...
        expected = DataFrame(
            {"A": date_range("2013-01-01 09:00:00",
    ...
    def test_naive_datetimeindex_roundtrip(self):
        ...
        assert expected.to_sql("foo_table", self.conn,
    ...
    def test_date_parsing(self):
        ...
        df = sql.read_sql_table(
            "types", self.conn, parse_dates={"IntDateCol": "s"})
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
    ...
    def test_dtype(self):
        ...
        # GH9083
        assert df.to_sql("dtype_test3", self.conn,
                         dtype={"B": String(10)}) == 2
        meta.reflect(bind=self.conn)
    ...
    def test_double_precision(self):
        ...
        assert np.round(df["f64"].iloc[0], 14) == np.round(
            res["f64"].iloc[0], 14)
    ...
    def test_read_sql_dtype_backend(self, string_storage, func, dtype_backend):
        ...
        with pd.option_context("mode.string_storage", string_storage):
            ...
            expected = self.dtype_backend_expected(
                string_storage, dtype_backend)
...
class TestSQLiteAlchemy(_TestSQLAlchemy):
    ...
    def test_row_object_is_named_tuple(self):
        ...
        with Session() as session:
            ...
            assert (
                df.to_sql("test_frame", con=self.conn,
...
class TestPostgreSQLAlchemy(_TestSQLAlchemy):
    ...
    def test_schema_support(self):
        ...
        assert (
            df.to_sql("test_schema_other", self.conn,
                      index=False, schema="other") == 2
        )
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def test_datetime_time(self, tz_aware):
        ...
        if not tz_aware:
            ...
            tz_dt = date_range("2013-01-01 09:00:00",
                               periods=2, tz="US/Pacific")
            tz_times = Series(tz_dt.to_pydatetime()).map(
                lambda dt: dt.timetz())
    ...
    def _get_index_columns(self, tbl_name):
        ...
        for ix_name in ixs.name:
            ix_info = sql.read_sql_query(
                f"PRAGMA index_info({ix_name})", self.conn)
...
class TestXSQLite:
    ...
    def test_basic(self, sqlite_buildin):
        ...
        assert (
            sql.to_sql(frame, name="test_table",
                       con=sqlite_buildin, index=False) == 30
        )
        ...
        assert (
            sql.to_sql(frame2, name="test_table2",
                       con=sqlite_buildin, index=False)
            == 30
        )
        result = sql.read_sql(
    ...
    def test_keyword_as_column_names(self, sqlite_buildin):
        ...
        assert sql.to_sql(df, con=sqlite_buildin,
                          name="testkeywords", index=False) == 5
    ...
    def test_onecolumn_of_integer(self, sqlite_buildin):
        ...
        assert sql.to_sql(mono_df, con=sqlite_buildin,
                          name="mono_df", index=False) == 2

...
class TestS3:
    ...
    def test_parse_public_s3n_bucket(self, tips_df, s3so):
        ...
        df = read_csv("s3n://pandas-test/tips.csv",
    ...
    def test_parse_public_s3a_bucket(self, tips_df, s3so):
        ...
        df = read_csv("s3a://pandas-test/tips.csv",
                      nrows=10, storage_options=s3so)
        assert isinstance(df, DataFrame)
        assert not df.empty
        tm.assert_frame_equal(tips_df.iloc[:10], df)
    ...
    def test_read_csv_chunked_download(self, s3_resource, caplog, s3so):
        ...
        s3_resource.Bucket(
            "pandas-test").put_object(Key="large-file.csv", Body=buf)
        ...
        with caplog.at_level(logging.DEBUG, logger="s3fs"):
            read_csv("s3://pandas-test/large-file.csv",
                     nrows=5, storage_options=s3so)

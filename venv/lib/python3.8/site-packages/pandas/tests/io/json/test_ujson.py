...
class TestUltraJSONTests:
    ...
    def test_encode_object(self):
        ...
        assert ujson.decode(ujson.encode(test_object)) == {
            "a": 1, "b": 2, "d": 4}
...
class TestNumpyJSONTests:
    ...
    def test_0d_array(self):
        # gh-18878
        msg = re.escape(
            "array(1) (0d array) is not JSON serializable at the moment")
        with pytest.raises(TypeError, match=msg):
...
class TestPandasJSONTests:
    ...
    def test_index(self):
        ...
        output = Index(ujson.decode(ujson.encode(
            i, orient="values")), name="index")
        tm.assert_index_equal(i, output)

        output = Index(ujson.decode(ujson.encode(
            i, orient="records")), name="index")
        tm.assert_index_equal(i, output)

        output = Index(ujson.decode(
            ujson.encode(i, orient="index")), name="index")
        tm.assert_index_equal(i, output)
    ...
    def test_decode_floating_point(self, sign, float_number):
        float_number *= sign
        tm.assert_almost_equal(float_number, ujson.loads(
            str(float_number)), rtol=1e-15)

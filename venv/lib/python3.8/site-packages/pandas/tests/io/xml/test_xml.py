...
def test_file_only_attrs(datapath, parser):
    ...
    df_iter = read_xml(filename, parser=parser,
...
<shapes>
  ...
  <shape>
    ...
    df_xpath = read_xml(xml, xpath=".//shape",
                        parser=parser, names=["name", "group"])
...
def test_empty_stylesheet(val):
    ...
    with pytest.raises(
        XMLSyntaxError, match=(
            "Document is empty|Start tag expected, '<' not found")
    ):

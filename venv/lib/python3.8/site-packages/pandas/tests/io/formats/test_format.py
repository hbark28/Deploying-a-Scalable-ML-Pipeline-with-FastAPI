...
class TestDataFrameFormatting:
    ...
    def test_show_counts(self, row, columns, show_counts, result):
        ...
        df = DataFrame(1, columns=range(
            10), index=range(10)).astype({1: "float"})
        df.iloc[1, 1] = np.nan
    ...
    def test_wide_repr_multiindex(self):
        with option_context("mode.sim_interactive", True, "display.max_columns", 20):
            ...
            max_cols = get_option("display.max_columns")
            df = DataFrame(tm.rands_array(
                25, size=(10, max_cols - 1)), index=midx)
...
"""
    ...
    def test_repr_html_wide_multiindex_cols(self):
        ...
        df = DataFrame(tm.rands_array(
            25, size=(10, len(mcols))), columns=mcols)
    ...
    def test_repr_html_long_multiindex(self):
        ...
        df = DataFrame(np.random.randn(max_L1 * 2, 2),
...
class TestPeriodIndexFormat:
    def test_period_format_and_strftime_default(self):
        ...
        per = pd.period_range(
            "2003-01-01 12:01:01.123456789", periods=2, freq="n")
        formatted = per.format()
    ...
    def test_period_custom(self):
        ...
        # 6 digits
        per = pd.period_range(
            "2003-01-01 12:01:01.123456", periods=2, freq="u")
        formatted = per.format(date_format="%y %I:%M:%S (ms=%l us=%u ns=%n)")
...
class TestDatetimeIndexFormat:
    ...
    def test_date_tz(self):
        ...
        formatted = pd.to_datetime(
            [datetime(2013, 1, 1), NaT], utc=True).format()
        assert formatted[0] == "2013-01-01 00:00:00+00:00"

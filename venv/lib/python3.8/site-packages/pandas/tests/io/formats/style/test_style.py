...
def test_hide_raises(mi_styler):
    ...
    with pytest.raises(ValueError, match=msg):
        mi_styler.hide(axis="index", subset="something",
...
def test_apply_map_header_mi(mi_styler, method, axis):
    ...
    result = getattr(mi_styler, f"{method}_index")(
        func[method], axis=axis)._compute()
...
class TestStyler:
    ...
    def test_multiple_render(self, df):
        # GH 39396
        s = Styler(df, uuid_len=0).applymap(
            lambda x: "color: red;", subset=["A"])
    ...
    def test_apply_subset(self, slice_, axis, df):
        ...
        result = df.style.apply(
            h, axis=axis, subset=slice_, color="baz")._compute().ctx
        expected = {
            (r, c): [("color", "baz")]
    ...
    def test_applymap_subset(self, slice_, df):
        result = df.style.applymap(
            lambda x: "color:baz;", subset=slice_)._compute().ctx
        expected = {
            (r, c): [("color", "baz")]
            for r, row in enumerate(df.index)
            for c, col in enumerate(df.columns)
    ...
    def test_hide_single_index(self, df):
        ...
        ctx4 = df.set_index("A").style.hide(
            axis="index")._translate(True, True)

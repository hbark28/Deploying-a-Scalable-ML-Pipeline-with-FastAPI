...
def test_apply_with_mutated_index():
    ...
    # A case for series
    expected = df["col1"].groupby(pd.Grouper(
        freq="M"), group_keys=False).apply(f)
...
def test_resample_groupby_with_label():
    ...
    df = DataFrame(index=index, data={
                   "col0": [0, 0, 1, 1, 2], "col1": [1, 1, 1, 1, 1]})
...
def test_resample_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    df1 = DataFrame({"key": "A", "date": dates,
                    "col1": range(15), "col_object": "val"})
...
def test_resample_no_index(keys):
    ...
    result = df.groupby(keys).resample(rule=pd.to_timedelta("00:00:01")).mean()
    expected = DataFrame(columns=["a", "b", "date"]
                         ).set_index(keys, drop=False)
...
def test_resample_no_columns():
    ...
    tm.assert_frame_equal(
        result, expected, check_index_type=not is_platform_windows())

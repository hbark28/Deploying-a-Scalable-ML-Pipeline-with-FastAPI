...
@pytest.mark.parametrize(
    "other", [Index(["three", "one", "two"]), Index(
        ["one"]), Index(["one", "three"])]
)
...
def test_join_midx_string():
    ...
    midx2 = MultiIndex.from_arrays(
        [Series(["a"], dtype=StringDtype()),
         Series(["c"], dtype=StringDtype())],
        names=["a", "c"],
    )
    result = midx.join(midx2, how="inner")
    expected = MultiIndex.from_arrays(
...
def test_join_dtypes(any_numeric_ea_dtype, val):
    # GH#49830
    midx = MultiIndex.from_arrays(
        [Series([1, 2], dtype=any_numeric_ea_dtype), [3, 4]])
    midx2 = MultiIndex.from_arrays(
...
def test_join_dtypes_all_nan(any_numeric_ea_dtype):
    ...
    midx2 = MultiIndex.from_arrays(
        [Series([1, 0, 0], dtype=any_numeric_ea_dtype),
...
def test_join_index_levels():
    # GH#53093
    midx = midx = MultiIndex.from_tuples(
        [("a", "2019-02-01"), ("a", "2019-02-01")])

...
class TestSliceLocs:
    ...
    def test_slice_locs_not_sorted(self):
        index = MultiIndex(
            levels=[Index(np.arange(4)), Index(
                np.arange(4)), Index(np.arange(4))],
            codes=[
...
def test_timestamp_multiindex_indexer():
    ...
    idx = MultiIndex.from_product(
        [
            date_range("2019-01-01T00:15:33", periods=100,
                       freq="H", name="date"),
            ["x"],
            [3],
        ]
    ...
    should_be = pd.Series(data=np.arange(
        24, len(qidx) + 24), index=qidx, name="foo")

...
class TestFrameLegend(TestPlotBase):
    ...
    def test_legend_false(self):
        ...
        ax = df.plot(legend=True, color={
                     "a": "blue", "b": "green"}, secondary_y="b")
    ...
    def test_df_legend_labels(self):
        ...
        df = DataFrame(np.random.randn(3, 3), columns=[
                       "a", "b", "c"], index=ind)
        df2 = DataFrame(np.random.randn(3, 3), columns=[
                        "d", "e", "f"], index=ind)
        df3 = DataFrame(np.random.randn(3, 3), columns=[
                        "g", "h", "i"], index=ind)
        ax = df.plot(legend=True, secondary_y="b")
        self._check_legend_labels(ax, labels=["a", "b (right)", "c"])
        ax = df2.plot(legend=False, ax=ax)

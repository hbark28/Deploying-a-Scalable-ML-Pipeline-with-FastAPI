...
class RowStringConverter:
    ...
    def get_strrow(self, row_num: int) -> str:
        ...
        is_cline_maybe_required = is_multirow and row_num < len(
            self.strrows) - 1
...
class LatexFormatter:
    ...
    def __init__(
        ...
        self.multicolumn_format = multicolumn_format
        self.multirow = multirow
        self.caption, self.short_caption = _split_into_full_short_caption(
            caption)
        self.label = label
        self.position = position
...
def _convert_to_bold(crow: Sequence[str], ilevels: int) -> list[str]:
    ...
    return [
        f"\\textbf{{{x}}}" if j < ilevels and x.strip() not in [
            "", "{}"] else x

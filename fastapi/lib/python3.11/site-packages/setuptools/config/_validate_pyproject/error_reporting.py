...
_NEED_DETAILS = {"anyOf", "oneOf", "anyOf",
                 "contains", "propertyNames", "not", "items"}
...
class ValidationError(JsonSchemaValueException):
    ...
    def _from_jsonschema(cls, ex: JsonSchemaValueException):
        ...
        obj = cls(str(formatter), ex.value,
                  formatter.name, ex.definition, ex.rule)
        debug_code = os.getenv(
            "JSONSCHEMA_DEBUG_CODE_GENERATION", "false").lower()
        ...
        if path[-1] == "type" and not self._is_property(path):
            ...
            return (
                f"[{', '.join(type_)}]" if isinstance(
                    value, list) else cast(str, type_)
            )
        return repr(value)
